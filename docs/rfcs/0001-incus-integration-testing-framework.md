...
PF_KEY-to-XFRM migration (issue #4) is a kernel-interface replacement that is
only verifiable by observing real kernel SPD/SAD state under real negotiation
traffic. Distribution packaging across five Ubuntu releases has already
regressed silently more than once. Today, integration testing is manual,
unrepeatable, and not run on every change.
...
| Test Orchestration | Resolves topology + scenario; sequences layers through lifecycle; aggregates results. | Talk to Incus directly or contain verification knowledge. |
...
| Test Execution | Drives scenario steps; produces observations. | Decide pass/fail. |
| Verification | Asserts expected outcomes against observations. | Provision, execute, or collect artifacts. |
...
`docs/suites/kernelpaws_testing.md` (Categories A/B/C/D). That harness
operates directly on the host kernel with `CAP_NET_ADMIN`, without container
topologies.
...
rekeying, package lifecycle, cross-distribution interoperability). Future RFCs
...
- [ ] At least `site-to-site` topology and `ike-negotiation-basic` scenario
  are implemented before this framework covers any specific issue.
...
failure, the full bundle is always collected.
...
- **Fault injection** — packet loss, latency, reordering via `tc netem`.

## References

- `docs/rfcs/README.md` — the project's RFC process.
- `test/Makefile.am` and `test/README.md` — existing unit test suite.
- `docs/suites/kernelpaws_testing.md` (branch `rfc-002/kernelpaws-prototype`)
