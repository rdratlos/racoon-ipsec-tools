...
#
# ISSUE #90 LIVE VERIFICATION (revision 4): steps 3b/4b/5c add a live,
# end-to-end check of the phase1-down.sh FIFO generation-matching fix
# (doc/dev/teardown-investigation.md's "related finding", filed as issue
# #90, fixed via IKE_COOKIE -- src/racoon/isakmp.c's script_hook() and
# racoon-hook-lib.sh's rhook_state_own_generation()). Reproducing the
# original bug's exact live conditions (two genuinely concurrent Phase 1
# negotiations to the same real gateway, one abandoned) is impractical
# against a production-adjacent gateway -- most reject or replace a
# second SA for the same identity outright. Instead a synthetic orphan
# generation is injected directly under $STATE_DIR before this run's own
# connect, with a fake IKE_COOKIE no real negotiation could ever produce
# and a synthetic undo_command that only touches a marker file under
# $OUT -- inert if it is ever actually run, so this is safe against a
# live host regardless of which way the check comes out. See the
# comments above peer_state_prefix()/peer_max_generation() below for the
# full mechanics. This piggybacks on the same connect/disconnect cycle
# steps 4-6 already drive -- it does not add a second one.
#
# Usage:
...
#
# VPN_GATEWAY CAVEAT (pre-existing, shared by step 5b and the new step
# 3b/4b/5c checks): hook-state filenames are built from racoon's own
# REMOTE_ADDR, the resolved numeric peer address -- not necessarily the
# same string as VPN_GATEWAY if you pass a hostname. Pass VPN_GATEWAY as
# the literal address racoon.conf's remote{} block resolves to (check
# 04-hook-state-listing.log's actual filenames against what you passed
# if a step ever reports "none found" unexpectedly) for these checks to
# line up exactly.
#
