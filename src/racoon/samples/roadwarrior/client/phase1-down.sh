...
# authoritative signal is NetworkManager's own D-Bus RcManager property
# (file layout can't be trusted: even NM's "symlink" mode writes a plain
# file when /etc/resolv.conf started out as one), falling back to file
# inspection only when D-Bus/busctl is unavailable (see phase1-up.sh for
# the full rationale).
# --------------------------------------------------------------------------
nm_dbus_prop() {
