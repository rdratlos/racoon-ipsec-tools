...
detect_resolver() {
	...
	# Not a symlink to a known backend's generated file.  Some distros
	# (e.g. Manjaro) configure NetworkManager with rc-manager=file: NM
	# writes straight into /etc/resolv.conf instead of symlinking it, so
	# the symlink check above never fires even though NM is genuinely the
	# writer.  Each backend stamps its output with a recognizable header
	# comment -- check content before falling back to guessing from which
	# services happen to be running, which is weaker: a backend can be
	# active without being the one that actually owns the file.
